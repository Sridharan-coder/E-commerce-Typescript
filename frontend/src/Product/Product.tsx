...
const Product = () => {
    ...
    const handleAddCart = async () => {
        ...
        // navigate(`/viewcart/${product.p_id}`)
    }

    const handleBuyNow = () => {
        async function fetchData() {
            await axios.get(`http://localhost:3321/getProductById/${searchParams.get("id")}`)
    ...
    const handleWhishlist = async (data:productInfo) => {
        const tempBuyer = buyerInfo;
        if (!tempBuyer.u_whitelist.includes(data.p_id as number)) {
            tempBuyer.u_whitelist = [...tempBuyer.u_whitelist, data.p_id as number]
